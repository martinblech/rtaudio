(() => {
    ...
    draw() {
      ...
      if (spectrum) {
        ...
        if (this.mouseCoordinates != null) {
          ...
          const freq = this.binToFreq(
            bin,
            spectrum.length,
            this.audioFrame.sampleRate
          );
          ...
          const binLabelX =
            x + 4 + maxLabelWidth < this.canvas.width
              ? x + 2
              : x - binLabelWidth - 2;
          const freqLabelX =
            x + 4 + maxLabelWidth < this.canvas.width
  ...
  socket.on("audioframe", (frame) => {
    for (let field of ["samples", "fft", "absoluteFft"]) {
      frame[field] = new Float32Array(frame[field]);
    }
    for (let bandName of ["bass", "mid", "high"]) {
      const band = frame[bandName];
      band.samples = new Float32Array(band.samples);
    }
    window.lastAudioFrame = frame;
    renderers.forEach((renderer) => {
      renderer.audioFrame = frame;
    });
  });
  ...
  $(() => {
    renderers.push(
      new SamplesRenderer(addVizCanvas("samples", 600, 100), (frame) => {
        return frame.samples;
      })
    );
    renderers.push(
      new SpectrumRenderer(addVizCanvas("spectrum", 600, 100), (frame) => {
        return frame.absoluteFft;
      })
    );
    renderers.push(
      new HistoryRenderer(
    ...
    renderers.push(
      new HistoryRenderer(
        addVizCanvas("peak", 600, 100),
        (frame) => {
          return frame.normalizedPeakFast;
        },
        300
      )
    );
    renderers.push(
      new HistoryRenderer(
        addVizCanvas("bass", 600, 100),
        (frame) => {
          return frame.bass.normalizedPeakFast;
        },
        300
      )
    );
    renderers.push(
      new HistoryRenderer(
        addVizCanvas("mid", 600, 100),
        (frame) => {
          return frame.mid.normalizedPeakFast;
        },
        300
      )
    );
    renderers.push(
      new HistoryRenderer(
        addVizCanvas("high", 600, 100),
        (frame) => {
          return frame.high.normalizedPeakFast;
        },
        300
      )
    );
